...
class GeometryStatistics():
   ...
   distances, angles, and dihedral cosines and sines.
    ...
    def __init__(self, data, custom_feature_tuples=None, backbone_inds=None,
                 ...
                 adjacent_backbone_bonds=True, mol = 'fibri'):
        ...
        self.mol = mol
        self.g = Geometry(method='numpy',mol = self.mol)
        ...
        if self.temperature is not None:
            if self.mol == 'chi' or self.mol == 'chi_cacb':
                self.beta = JPERKCAL/KBOLTZMANN/AVOGADRO/self.temperature # unit mole/kcal
            elif self.mol =='fibri':
                self.beta = 1000/KBOLTZMANN/AVOGADRO/self.temperature # unit mole/kJ
            else:
        ...
        if get_all_distances:
            (self._pair_order,
             self._adj_backbone_pairs) = self.g.get_distance_indices(self.n_beads,
                                                                self.backbone_inds,
                                                                self._backbone_map)
    ...
    def _get_distances(self):
        ...
        """
        self.distances = self.g.get_distances(
        ...
        if self.get_redundant_distance_mapping:
            self.redundant_distance_mapping = self.g.get_redundant_distance_mapping(
                self._distance_pairs)

    def _get_angles(self):
        """Obtains all planar angles for the three-bead indices provided.
        """
        self.angles = self.g.get_angles(self._angle_trips, self.data)
    ...
    #     """
    #     (self.dihedral_cosines,
    #         self.dihedral_sines) = self.g.get_dihedrals(self._dihedral_quads, self.data)
    ...
    def _get_dihedrals(self):
        """Obtains all dihedral angles for the four-bead indices provided.
        """
        # self.dihedral_angles,_,_ = self.g.get_dihedrals(self._dihedral_quads, self.data)
        self.dihedral_angles = self.g.get_dihedrals(self._dihedral_quads, self.data)

        self.descriptions['Dihedral_angles'].extend(self._dihedral_quads)

        self._get_stats(self.dihedral_angles, 'Dihedral_angles')
        self.order += ['Dihedral_angles']
    ...
    def return_indices(self, features):
        ...
        if isinstance(features, str):
            ...
            if features in ["Distances", "Angles", "Dihedral_angles"]:
