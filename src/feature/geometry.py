...
class Geometry():
    ...
    def __init__(self, method='torch', device=torch.device('cpu'), mol = 'fibri'):
        self.device = device
        ...
        self.mol = mol
        if self.mol == 'fibri':
            self.cg_topo = md.load_psf('cgnet/feature/cg_3GHG_autopsf.psf')
        elif self.mol == 'chi':
            self.cg_topo = md.load_psf('cgnet/feature/cg_chignolin.psf')
        elif self.mol == 'chi_cacb':
            self.cg_topo = md.load_psf('cgnet/feature/cg_chignolin_cacb.psf')
        else:
            raise RuntimeError("Please specify the structure of the molecule!")
    ...
    # def get_dihedrals(self, dihed_inds, data, tan = True):
    #     """Calculates dihedrals in a vectorized fashion.
    ...
    #         dihedral_angles = self.arctan(dihedral_sines/dihedral_cosines)
        
    #     self.check_for_nans(dihedral_angles, 'dihedral angles')

    #     return dihedral_angles,dihedral_sines,dihedral_cosines
        
    def get_dihedrals(self, dihed_inds, data, tan = True):
        """Modify from MDtraj
        Specifically for Fibrinogen
        """
        if self.method == 'torch':
            traj = data.detach().numpy()
        else:
            traj = data
        if self.mol == 'fibri':
            cg_traj = md.core.trajectory.Trajectory(traj, self.cg_topo, 
                                            unitcell_angles=np.tile(np.array([90., 90., 90.]), (traj.shape[0],1)), 
                                            unitcell_lengths=np.tile(np.array([45.28303, 15.10107, 15.45354]), (traj.shape[0],1)))
        elif self.mol == 'chi' or self.mol == 'chi_cacb':
            cg_traj = md.core.trajectory.Trajectory(traj, self.cg_topo, 
                                            unitcell_angles=np.tile(np.array([90., 90., 90.]), (traj.shape[0],1)), 
                                            unitcell_lengths=np.tile(np.array([3.9972   , 3.9941003, 3.9935002]), (traj.shape[0],1)))
        else:
            raise RuntimeError("Please specify the structure of the molecule!")
        dihedral_angles = md.compute_dihedrals(cg_traj,dihed_inds)
        
        if self.method == 'torch':
            return torch.tensor(dihedral_angles)
        else:
