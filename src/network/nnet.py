...
import numpy as np
from .priors import ZscoreLayer, HarmonicLayer, RepulsionLayer, HarmonicLayer_L, RepulsionLayer_L, LJ_L, FENE_bond_L, REB_angle_L
...
class CGnet_FP(nn.Module):
    ...
    def __init__(self, arch, criterion, feature=None, priors=None, bond_in_feature = None, resting_para = None, device = None):
        super(CGnet_FP, self).__init__()
        zscore_idx = 1
        for layer in arch:
            if isinstance(layer, ZscoreLayer):
                self.register_buffer('zscores_{}'.format(zscore_idx),
        ...
        self.criterion = criterion
        self.feature = feature
        self.bond_in_feature = bond_in_feature
        self.resting_para = resting_para
        self.device = device
        
        self.pairnum = 0
        self.bnum = len(self.bond_in_feature)
        self.anum = 0
        self.dnum = 0
        for i in range(len(self.feature.feature_tuples)):
            if len(self.feature.feature_tuples[i]) == 2:
                self.pairnum +=1
            if len(self.feature.feature_tuples[i]) == 3:
                self.anum +=1
            if len(self.feature.feature_tuples[i]) == 4:
                self.dnum +=1
        
        self.alpha0 = 1.2
        # self.alpha0 = nn.Parameter(torch.tensor(1.2, requires_grad=True))
        self.D0= nn.ParameterList([nn.Parameter(torch.randn(1,requires_grad=True)) for i in range(self.pairnum)])
        self.kb = nn.ParameterList([nn.Parameter(torch.randn(1,requires_grad=True)) for i in range(self.bnum)])
        self.ka = nn.ParameterList([nn.Parameter(torch.randn(1,requires_grad=True)) for i in range(self.anum)])
        self.kd = nn.ParameterList([nn.Parameter(torch.randn(1,requires_grad=True)) for i in range(self.dnum)])
        
        # self.D0= nn.Parameter(torch.randn(self.pairnum,requires_grad=True))
        # self.kb = nn.Parameter(torch.randn(self.bnum,requires_grad=True))
        # self.ka = nn.Parameter(torch.randn(self.anum,requires_grad=True))
        # self.kd = nn.Parameter(torch.randn(self.dnum,requires_grad=True))
    ...
    def forward(self, coordinates, embedding_property=None):
        ...
        energy_nb = torch.zeros(energy.shape).to(self.device)
        energy_b = torch.zeros(energy.shape).to(self.device)
        energy_a = torch.zeros(energy.shape).to(self.device)
        energy_d = torch.zeros(energy.shape).to(self.device)
        
        for i in range(feature_output.shape[0]):
            for j in range(self.pairnum):
                energy_nb[i] += torch.mul(torch.square(self.D0[j][0]), torch.exp(-2*self.alpha0*(feature_output[i][j]-self.resting_para[j])) -
                2*torch.exp(-self.alpha0*(feature_output[i][j]-self.resting_para[j])))
            for j in range(self.bnum):
                energy_b[i] += torch.mul(torch.square(self.kb[j][0]), torch.square(feature_output[i][self.bond_in_feature[j]] - self.resting_para[self.bond_in_feature[j]]))
            for j in range(self.anum):
                energy_a[i] += torch.mul(torch.square(self.ka[j][0]), torch.square(feature_output[i][j+self.pairnum] - self.resting_para[j+self.pairnum]))
            for j in range(self.dnum):
                energy_d[i] += torch.mul(torch.square(self.kd[j][0]), torch.square(feature_output[i][j+self.pairnum+self.anum] - self.resting_para[j+self.pairnum+self.anum]))
                
                
        theory_energy = energy_nb + energy_b + energy_a + energy_d             
        theory_force = torch.autograd.grad(-torch.sum(theory_energy),
                                    coordinates,
                                    create_graph=True,
                                    retain_graph=True)
        
        return energy, force[0], theory_energy, theory_force[0]
...
class CGnet_LP(nn.Module):
    ...
    def __init__(self, arch, criterion, feature=None, priors=None):
        ...
        if priors:
            ...
            # repul_idx = 1
            # for layer in self.priors:
            #     if isinstance(layer, HarmonicLayer_L):
            #         self.register_parameter('harmonic_params_{}'.format(harm_idx),
            #                              layer.harmonic_parameters)
            #         harm_idx += 1
            #     if isinstance(layer, RepulsionLayer_L):
            #         self.register_parameter('repulsion_params_{}'.format(repul_idx),
            #                              layer.repulsion_parameters)
            #         repul_idx += 1
        else:
            self.priors = None
